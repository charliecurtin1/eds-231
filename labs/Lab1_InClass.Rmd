...
baseurl <- paste0("http://api.nytimes.com/svc/search/v2/articlesearch.json?q=", 
                  term1, term2, "&begin_date=", begin_date,
                  "&end_date=", end_date, "&facet_filter=true&api-key=", API_KEY, sep="")
...
for(i in 1:maxPages){
  ...
  pages[[i+1]] <- nytSearch 
  # tell for loop to wait 12 seconds between retreiving pages. The API only returns a certain number of results within a certain amount of time
  Sys.sleep(12)
}
```
...
nyt_df %>%
  mutate(pubDay=gsub("T.*","",response.docs.pub_date)) %>% # remove the time from the date column
  group_by(pubDay) %>%
  summarise(count=n()) %>%
  filter(count >= 2) %>%
  ggplot() +
...
names(nyt_df)
paragraph <- head(nyt_df[,1:6])

# tokenization- returns the total number of words in all of the lead paragraphs in our dataframe 
tokenized <- nyt_df %>% 
  filter(response.docs.news_desk != "Games") %>% 
  # unnest_tokens adds a column "word" with each unique word
...
```{r stop-words}
# load stop words- stop words are basically words that are common but don't tell us anything useful
data(stop_words)
...
# clean tokens- from words, remove all numbers, all dates
clean_tokens <- str_remove_all(tokenized$word, "[:digit:]")

# clean tokens, remove apostrophes
clean_tokens <- gsub("'s", "", clean_tokens)

# add clean tokens as a column in tokenized
tokenized$clean <- clean_tokens

# create a new dataframe, removing all of rows with empty words. Empty bc we removed numbers 
