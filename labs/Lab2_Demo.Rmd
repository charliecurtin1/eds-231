...
setwd() #where the .docxs live

post_files <- list.files(pattern = ".docx", path = here("data", "Post"),
...
ggplot(sent_article, aes(x = ID)) +
  geom_col(aes(y = negative), stat = "identity", fill = "darkred") +
  geom_col(aes(y = positive), stat = 'identity', fill = "cornflowerblue") +
  theme_bw() +
  labs(title = "Sentiment Analysis of Deforestation Articles")
```
